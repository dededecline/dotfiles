...
parse_displays_toml() {
  ...
  [[ -f "$config" ]] || {
    echo "displays.toml not found: $config"
    return 1
  }
  MONITOR_NAMES=()
  local section=""
...
build_dp_arg() {
  ...
  _key="MONITOR_${name}_res"
  res="${!_key}"
  _key="MONITOR_${name}_hz"
  hz="${!_key}"
  _key="MONITOR_${name}_color_depth"
  color_depth="${!_key}"
...
apply_display_profile() {
  ...
  if [[ ${#externals[@]} -eq 0 ]]; then
    ...
    if $has_builtin; then
      ...
      _key="MONITOR_${ext}_dual_origin"
      dual_origin="${!_key}"
