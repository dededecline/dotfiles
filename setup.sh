...
# Preprocess files with machine markers, e.g. `// @machine:MacBook-Pro-HF7C7K3WJX` / `// @end:MacBook-Pro-HF7C7K3WJX`
# (JSONC) or `# @machine:MacBook-Pro-HF7C7K3WJX` / `# @end:MacBook-Pro-HF7C7K3WJX` (TOML / shell).
...
preprocess_machine_markers() {
  ...
  local groups
  # Explicit check: `set -e` is suppressed inside `if`/`&&` callers, so an
  # unchecked assignment here would emit a stripped config instead of failing.
  groups=$(get_machine_groups "$MACHINE_HOSTNAME") || return 1
...
configure_claude() {
  ...
  local rendered="${output}.rendered"
  preprocess_machine_markers "$base" "$tmp" "//" || return 1
  # shellcheck disable=SC2016
  if ! sed -E 's/,([[:space:]]*[]{}])/\1/g' "$tmp" | jq . | envsubst '$HOME' >"$rendered"; then
    print_error "Claude settings: generation failed, keeping previous settings.json"
    rm -f "$tmp" "$rendered"
    return 1
  fi
  mv "$rendered" "$output"
