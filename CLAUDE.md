  ...
  **generated** at setup time by concatenating the label Brewfiles the
  ...
  `CLAUDE_CONFIG_DIR`). `codex/config.toml`, `codex/AGENTS.md`, and
  `codex/prompts/` are symlinked into `~/.codex/` as fallbacks for shells
  or tools that read Codex's default home.
  ...
  To modify settings: edit `codex/config.source.toml`, then run
  `./setup.sh`.
