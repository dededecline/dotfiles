  ...
  • Generates Claude settings from profile-filtered JSONC
  • Creates symlinks
  ...
  `claude/settings.json` is generated by `setup.sh` using the same
  `@profile:work` / `@end:work` marker pattern as the Brewfile. The Laurel
  plugin (work MCP tools) is only included on the work profile.
