  ...
  **generated** at setup time by concatenating the label Brewfiles the
  ...
  │   ├── laptop/Brewfile      # Laptop machines (athena, Naomi-Klein-MacBook-Pro)
  │   ├── personal/Brewfile    # Personal machines (athena only)
  │   ├── creative/Brewfile    # Gaming / DAWs (athena only)
  │   ├── infra/Brewfile       # Infrastructure machines (Naomi-Klein-MacBook-Pro, nyx)
  │   ├── server/Brewfile      # Server machines (nyx only)
  │   └── work/Brewfile        # Work group (Naomi-Klein-MacBook-Pro only)
  ...
  • `creative` is athena-only (gaming + DAW content that shouldn't reach
    Naomi-Klein-MacBook-Pro or nyx).
  ...
  • Python version management is split by group: `uv` lives in `personal`
    (athena), `mise` in `work` (Naomi-Klein-MacBook-Pro). `pyenv` was removed. The shared `all`
  ...
  • `// @machine:Naomi-Klein-MacBook-Pro` — only on Naomi-Klein-MacBook-Pro
  • `// @machine:laptop` — on Naomi-Klein-MacBook-Pro + athena
  ...
  `codex/config.source.toml` is the source of truth for OpenAI Codex CLI
  settings. `codex/config.toml` is generated by `setup.sh` using the same
  `@machine:<name>` / `@end:<name>` markers as Claude, with `#` comment
  prefixes (TOML-native) instead of `//`:
  • `# @machine:Naomi-Klein-MacBook-Pro` — only on Naomi-Klein-MacBook-Pro
  • `# @machine:athena` — only on athena
  • `# @machine:laptop` — on Naomi-Klein-MacBook-Pro + athena

  `CODEX_HOME=$HOME/.config/codex` is exported in `fish/config.fish` so
  Codex reads the generated config directly from this dir (mirrors
  `CLAUDE_CONFIG_DIR`). `codex/config.toml`, `codex/hooks.json`,
  `codex/AGENTS.md`, and `codex/prompts/` are symlinked into `~/.codex/`
  as fallbacks for shells or tools that read Codex's default home.

  `codex/hooks.json` is also generated by `configure_codex`, from
  `codex/hooks.source.json`: the hook command paths use a literal `$HOME`
  that `envsubst` expands to the current account's home per machine.
  The generated `codex/hooks.json` is gitignored; edit
  `codex/hooks.source.json` to change hook paths.

  Install: `cask "codex"` in `.system/profiles/labels/laptop/Brewfile`
  (laptop-only, mirroring `cask "claude"`).

  Auth: run `codex login` interactively once per machine (no API-key
  injection; uses ChatGPT subscription or API key written to
  `codex/auth.json`, which is gitignored).
  ...
  Remote desktop and SSH access from laptops (Naomi-Klein-MacBook-Pro, athena) to server (nyx)
  ...
  **Laptops (Naomi-Klein-MacBook-Pro, athena):**
  ...
  **Manual steps — laptops (Naomi-Klein-MacBook-Pro, athena):**
  ...
  **Claude Code auth:** Uses `apiKeyHelper` in `~/.claude/settings.json` to read
  ...
  generated from `codex/hooks.source.json`):
