...
(gaming + DAW content that shouldn't reach Naomi-Klein-MacBook-Pro or nyx). •
...
`work` (Naomi-Klein-MacBook-Pro). `pyenv` was removed. The shared `all` group
...
instead of `//`: • `# @machine:Naomi-Klein-MacBook-Pro` — only on
Naomi-Klein-MacBook-Pro • `# @machine:athena` — only on athena •
`# @machine:laptop` — on Naomi-Klein-MacBook-Pro + athena
...
Remote desktop and SSH access from laptops (Naomi-Klein-MacBook-Pro, athena) to
