#!/usr/bin/env bash
#
# sync-theme.sh - Sync theme colors from themes/theme.toml to all tool configs
#
...
#
# Usage:
#   ./setup/sync-theme.sh           # Run from dotfiles root
...
# Palette source: ~/.config/themes/catppuccin-${FLAVOR}/palette.toml
...
sync_git_config() {
    local file="$DOTFILES/templates/git-config.tpl"
    [[ -f "$file" ]] || return 0
    ...
    print_status "templates/git-config.tpl: updated delta theme"
}
...
sync_flavor_strings() {
    ...
    if [[ -f "$file" ]]; then
        sed -i '' "s|include ../themes/catppuccin-.*/kitty.conf|include ../themes/catppuccin-${FLAVOR}/kitty.conf|" "$file"
    ...
    file="$DOTFILES/templates/glow.yml.tpl"
    if [[ -f "$file" ]]; then
        sed -i '' "s|glow/catppuccin-.*\.json|glow/catppuccin-${FLAVOR}.json|" "$file"
        print_status "templates/glow.yml.tpl: style → catppuccin-${FLAVOR}"
    fi
}
...
check_upstream_themes() {
    local upstream_files=(
        "themes/catppuccin-${FLAVOR}/kitty.conf"
        "themes/catppuccin-${FLAVOR}/bat.tmTheme"
